...
select avg(price) as price_KIA from cars where model='KIA';
...
select count(*) as ppp, model from cars  group by model having count(model) order by ppp desc limit 1;
#
